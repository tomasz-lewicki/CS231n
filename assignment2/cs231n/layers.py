...
def relu_forward(x):
    ...
    out = np. maximum(0, x) #imo that should be cached too
    cache = x
...
def relu_backward(dout, cache):
    ...
    ###########################################################################
    scores = np.maximum(0, x) #i feel that should be cached
    dx = dout
    dx[scores == 0] = 0
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = (np.random.rand(*x.shape) < p) / p # 1 means keep
        out = x * mask
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_backward_naive(dout, cache):
    ...
    (N, C, H, W)= x.shape
    (F, _, Hf, Wf) = w.shape
    (_, Fout, Hout, Wout) = dout.shape
    
    #recreate the padded input
    ...
    dw = np.zeros_like(w)
    db = np.zeros_like(b)
    #dw = convolve dout over x_pad (dout is filter then)
    ...
    for f in range(F):
        for c in range(C):
            for hf in range(Hf):
                for wf in range(Wf):
                    dw[f,c,hf,wf] += np.sum(x_pad[:, c, hf*s:hf*s+Hout, wf*s:wf*s+Wout] * dout[:,f,:,:])
                    
                    
    for f in range(F):
        db[f] = np.sum(dout[:,f])
        
    #iterate over dout and calculate dx
    for n in range(N):
        for f in range(F):
            for hx in range(0, Hout):
                for wx in range(0, Wout):
                    dx_pad[n,:, hx*s:hx*s+Hf,wx*s:wx*s+Wf] += w[f] * dout[n,f,hx,wx]
...
def max_pool_forward_naive(x, pool_param):
    ...
    s = pool_param['stride']
    Ho, Wo = int(1+(H-Ph)/s), int(1+(W-Pw)/s)
    
    out = np.zeros((N, C, Ho, Wo))
    locations = np.array((N, C, Ho, Wo))
    for n in range(N):
        for c in range(C):
            for h in range(Ho):
                for w in range(Wo):
                    out[n,c,h,w] = np.max(x[n,c,h*s:h*s+Ph,w*s:w*s+Pw])
                    #locations[n,c,h,w] = np.argmax(x[n,c,h*s:h*s+Ph,w*s:w*s+Pw])
                    
    
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param, locations)
    return out, cache
        ...
        for c in range(C):
            for h in range(Ho):
                for w in range(Wo):
                    ...
                    poolslice = x[n,c,h*s:h*s+Ph, w*s:w*s+Pw]
                    max_loc = np.unravel_index(poolslice.argmax(), poolslice.shape)
                    dx[n,c, h*s + max_loc[0], w*s + max_loc[1]] = dout[n,c,h,w]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx, dgamma, dbeta, _ = batchnorm_backward(dout_new, cache)
